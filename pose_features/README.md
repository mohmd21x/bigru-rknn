...
## Project Overview
This module is a **pose-feature extraction utility** for downstream fall-detection and general ML workflows.
It converts **pose keypoint CSV files** (COCO 17 keypoints, per-frame) into **feature-rich CSV outputs** with engineered geometric and temporal signals.

- **What it is**: a **feature extractor** (offline / batch-friendly) that prepares ML-ready numeric features.
- **What it is not**: a fall decision system. It does **not** implement a fall state machine, tracking logic, or final fall event confirmation rules.

## Input / Output
- **Input**: CSV containing pose keypoints in **COCO-17** format, per frame and per person:
  - 17 keypoints × **(x, y, confidence)** per frame
  - plus basic identifiers (e.g., video name / frame index / timestamp / person id), depending on your export source
...
- **Per-person**: each person stream is processed independently (rows are associated with `person_id` when present).

## Features Description (high-level, grouped)
This extractor outputs a wide feature set. The exact column list may evolve, but it is broadly grouped as:
...
- **Temporal features**: velocity/acceleration-style features require a **previous valid frame**; the first valid frame (or gaps) may not have meaningful temporal values.
- **Validity flags**: some rows may be marked invalid via helper columns such as **`valid_pose`** (and related flags) to support filtering in analytics/ML.
- **Window-based features**: small **sliding windows** are used for certain stability/aggregate features; at the start of a sequence, window-based values may be unavailable or less stable until enough history is present.

## Limitations
- **No fall decision logic** (no final “fall/not-fall” output).
- **No state machine** (no confirmation timers or event-level gating).
- **No tracking logic** (no ID association across frames; assumes `person_id` is already provided if multi-person is present).
...
- If needed for debugging, add optional “rule-style” diagnostic columns in a separate layer (without merging a full fall detector/state machine into this extractor).
